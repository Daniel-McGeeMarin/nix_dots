...

{
  ...
  programs.zsh = {
    ...
    shellAliases = {
      ...
      nixswitch = "st=\"$(date +%s)\"; sudo nixos-rebuild switch --flake $HOME/nixos/#XiaNix --cores 8 --impure && notify-send 'updated' \"Took: $(($(date +%s)-$st))s\"";
      homeswitch = "st=\"$(date +%s)\"; home-manager switch --flake $HOME/nixos/#XiaNix --cores 8 --impure && notify-send 'updated' \"Took: $(($(date +%s)-$st))s\"";
      nixtest = "st=\"$(date +%s)\"; sudo nixos-rebuild test --fast --flake $HOME/nixos/#XiaNix --cores 8 --impure && notify-send 'updated' \"Took: $(($(date +%s)-$st))s\"";
